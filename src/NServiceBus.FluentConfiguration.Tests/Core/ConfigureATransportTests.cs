...
namespace NServiceBus.FluentConfiguration.Tests
{
    ...
    public class ConfigureATransportTests
    {
        ...
        {
            var configureTransport = new ConfigureNServiceBus().WithEndpoint("Test").WithTransport<LearningTransport>().WithRouting(r => {});
        ...
        public void ConfigureATransport_WithRouting_ProvidingConfigurationCallback_ConfigurationCallbackIsCalled()
        {
            ...
            var configureTransport = new ConfigureNServiceBus().WithEndpoint("Test").WithTransport<LearningTransport>().WithRouting(r => { configurationCallbackCalled = true; });
        ...
        {
            var configureTransport = new ConfigureNServiceBus().WithEndpoint("Test").WithTransport<LearningTransport>().WithRouting<DefaultRoutingConfiguration>();
        ...
        {
            var configurationCallbackCalled = false;
            var configureTransport = new ConfigureNServiceBus().WithEndpoint("Test").WithTransport<LearningTransport>().WithRouting<DefaultRoutingConfiguration>(cfg => { configurationCallbackCalled = true; });
